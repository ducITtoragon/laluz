<?php 
add_action( 'wp_ajax_mona_admin_ajax_update_notfound',  'mona_admin_ajax_update_notfound' ); // login
function mona_admin_ajax_update_notfound() {
    try {
        $formdata = isset( $_POST ) ? $_POST : [];
        // check class 
        $callBack = (new MonaSettingNotFound());
        ...
        if ( ! wp_verify_nonce( $nonce, $callBack->__nonce_key() ) ) {
            echo wp_send_json_error(
                [
                    'toast'    => [
                        'type'     => 'info',
                        'content'  => get_message_error( 'Đã xảy ra lỗi! Vui lòng đóng trình duyệt và mở tại.' ),
                        'duration' => 5000,
                    ],
                    'template' => '',
                ]
